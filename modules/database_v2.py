...
def get_connection():
    return sqlite3.connect('../backtest.db')
...
def save_strategy(strategy_id, name, description, asset_class, timeframe):
    ...
    c = conn.cursor()
    try:
        c.execute('INSERT INTO strategies VALUES (?, ?, ?, ?, ?)', (strategy_id, name, description, asset_class, timeframe))
        conn.commit()
...
def save_backtest_run(backtest_id, timestamp, strategy_id, params, start_date, end_date, initial_cash, final_value, window_id, experiment_id):
    conn = get_connection()
    c = conn.cursor()
    try:
        c.execute('INSERT INTO backtest_runs VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (backtest_id, timestamp, strategy_id, json.dumps(params), start_date, end_date, initial_cash, final_value, window_id, experiment_id))
        conn.commit()
...
def save_optimization_run(id, timestamp, strategy_id, optimize_metric, best_params, best_value, backtest_id, experiment_id):
    conn = get_connection()
    c = conn.cursor()
    try:
        c.execute('INSERT OR REPLACE INTO optimization_runs VALUES (?, ?, ?, ?, ?, ?, ?, ?)', (id, timestamp, strategy_id, optimize_metric, json.dumps(best_params), best_value, backtest_id, experiment_id))
        conn.commit()
...
def save_iteration(id, datetime, iteration, fixed_parameter, cmd_parameter, optimize_metric, target, backtest_id, window_id):
    conn = get_connection()
    c = conn.cursor()
    try:
        c.execute('INSERT INTO optimization_iterations VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', (id, datetime, iteration, json.dumps(fixed_parameter), json.dumps(cmd_parameter), optimize_metric, target, backtest_id, window_id))
        conn.commit()
        # print(f"Saved iteration: {id}")
    except sqlite3.IntegrityError as e:
        print(f"Error saving iteration {id}: {e}")
        raise
    finally:
        conn.close()
...
def save_performance_metrics(backtest_id, metrics_dict, scope):
    conn = get_connection()
    c = conn.cursor()
    try:
        for name, value in metrics_dict.items():
            c.execute('INSERT INTO performance_metrics (backtest_id, metric_name, value, scope) VALUES (?, ?, ?, ?)', (backtest_id, name, value, scope))
        conn.commit()
        # print(f"Saved performance metrics for backtest: {backtest_id}, scope: {scope}")
    except sqlite3.IntegrityError as e:
...
def save_composite_metrics(backtest_id, sharpe, sortino, calmar, profit_factor, composite_score, scope):
    conn = get_connection()
    c = conn.cursor()
    try:
        id = str(uuid.uuid4())
        c.execute('INSERT INTO composite_metrics VALUES (?, ?, ?, ?, ?, ?, ?, ?)', (id, backtest_id, sharpe, sortino, calmar, profit_factor, composite_score, scope))
        conn.commit()
        # print(f"Saved composite metrics for backtest: {backtest_id}, scope: {scope}")
    except sqlite3.IntegrityError as e:
...
def save_equity_curve(backtest_id, equity_data):
    ...
    c = conn.cursor()
    try:
        for dt, eq, ret in equity_data:
            c.execute('INSERT INTO equity_curve (backtest_id, datetime, equity, return) VALUES (?, ?, ?, ?)', (backtest_id, dt, eq, ret))
        conn.commit()
