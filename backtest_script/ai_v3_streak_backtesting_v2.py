...
from modules.broker.commission_helper import get_commission_class
import numpy as np
import pandas as pd
...
from modules.metrics_utils import *
...
class MyStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.entry_pending and not self.position:
            ...
            self.log(f'BUY {self.trade_id}, Size: {self.pending_size}, Price: %.2f, SL: %.2f, TP: %.2f' % (self.entry_price, self.stoploss_price, self.takeprofit_price))
            self.entry_pending = False
            self.pending_size = 0
...
def run_backtest(params, start_date, end_date, data_file, initial_cash,
                 broker_name='zerodha', product_type='CNC', strategy_name='ai_v3_streak', optimize_metric='Net Profit', experiment_id=None, window_id=None, **kwargs):

    # Derive intraday from product_type
    intraday = (product_type.upper() == 'MIS')
    ...
    cerebro.broker.addcommissioninfo(get_commission_class(broker_name, product_type, 'equity'))
...
if __name__ == "__main__":
    ...
    metrics, backtest_id, total_trades, won_trades, lost_trades, total_commission = run_backtest(params, start_date, end_date, args.data_file, args.initial_cash, args.broker_name, args.product_type, args.strategy_name, args.optimize_metric, args.experiment_id, args.window_id)
