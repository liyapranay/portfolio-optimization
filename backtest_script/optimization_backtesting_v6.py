...
def process_window(window_config, fixed_args, optimize_metric, variable_params, opt_run_id, experiment_id, strategy_file, logger, n_trials):
    """Process a single optimization window with true parallelization"""
    iteration, current_train_start, train_end, test_end = window_config
    ...
    # Optuna Optimization
    best_params, best_value, train_metrics, train_backtest_id = optuna_optimize(strategy_file, train_fixed_args, optimize_metric, window_id, n_trials=n_trials, n_jobs=1, logger=logger, variable_params=variable_params)
    ...
    train_doc = {
        ...
        "stock_symbol": fixed_args['data_file'].split("_")[-1].split(".")[0],
        "timeframe": '5minute',
        "params": best_params,
...
def optuna_optimize_strategy(strategy_file, fixed_args, optimize_metric, train_years, test_months, start_date, end_date, n_trials, n_jobs_parallel, experiment_name, logger, variable_params):
    ...
    # Prepare window configurations for parallel processing
    window_configs = []
    ...
    while current_train_start < end:
        ...
        window_configs.append((iteration, current_train_start, train_end, test_end))
        current_train_start = train_end - relativedelta(months=test_months)

    # Process windows in parallel using joblib (true multiprocessing)
    print(f"🚀 Processing {len(window_configs)} windows in parallel using joblib multiprocessing (n_jobs={n_jobs_parallel})...")
    parallel_start = time.time()
    window_results = Parallel(n_jobs=n_jobs_parallel, backend='multiprocessing')(
        delayed(process_window)(config, fixed_args, optimize_metric, variable_params, opt_run_id, experiment_id, strategy_file, logger, n_trials)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Optuna Walk-Forward Optimization for Trading Strategies v6.0 (True Parallel with Joblib)')
    ...
    parser.add_argument('--n_trials', type=int, default=20, help='Number of Optuna trials per window')
    parser.add_argument('--n_jobs_parallel', type=int, default=-1, help='Number of parallel cores for joblib (-1 = all cores)')
    ...
    results, global_params = optuna_optimize_strategy(args.strategy_file, fixed_args, args.optimize_metric, args.train_years, args.test_months, args.start_date, args.end_date, args.n_trials, args.n_jobs_parallel, experiment_name, logger, variable_params)
